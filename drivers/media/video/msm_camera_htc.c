...
static int msm_pmem_frame_vtop_lookup(unsigned long buffer,
...
static long msm_control(void __user *arg, struct msm_device_t *msm)
{
	unsigned long flags;
	//int timeout;
	long rc = 0;

	struct msm_ctrl_cmd_t ctrlcmd_t, *ctrlcmd;
	struct msm_queue_cmd_t *qcmd = NULL;
	...
	rc = wait_event_interruptible_timeout(msm->sync.ctrl_status_wait,
				!list_empty_careful(&msm->sync.ctrl_status_queue),
				MAX_SCHEDULE_TIMEOUT);

	CDBG_INFO("msm_control: rc = %ld\n", rc);
	if (list_empty_careful(&msm->sync.ctrl_status_queue)) {
		if (!rc)
			rc = -ETIMEDOUT;
		if (rc < 0) {
			CDBG_ERR("msm_control: wait_event error %ld\n", rc);
			goto fail;
		}
	}
	...
	if (!qcmd->command) { // __msm_control function in AOSP checked
		printk("qcmd->command is NULL!\n");
		ctrlcmd_t.type = 0xFFFF;
		ctrlcmd_t.length = 0xFFFF;
		ctrlcmd_t.status = 0xFFFF;
	} else {
		CDBG_INFO("msm_control: length = %d\n",
...
no_mem:
	...
	CDBG_INFO("msm_control: end rc = %ld\n", rc); /* debug, cn.fyodor */
	return rc;
}
...
static long msm_get_pic_proc(struct msm_ctrl_cmd_t *ctrl,
	struct msm_device_t *msm)
{
	unsigned long flags;
	long rc = 0;
	...
	if (tm > 0) {
		...
		if (rc == 0) {
			CDBG_WARING("msm_get_pic_proc timeout\n");
			return -ETIMEDOUT;
		}
	} else{
...
// andy add
// cn.fyodor
	rc = msm_get_pic_proc(&ctrlcmd_t, msm);
	if (rc < 0)
		return rc;
	...
	struct poll_table_struct *pll_table)
{
	...
	if (!list_empty(&pmsm->sync.prev_frame_q))
		/* frame ready */
...
mem_fail1:
	//if (qtype == MSM_CAMERA_MSG && vdata->evt_msg.len > 0) // cn.fyodor
	if (vdata->evt_msg.len > 0)
...
static long msm_open_proc(struct msm_device_t *msm)
{
	...
	if (msm->opencnt == 1)  {
		...
		if (msm->vfefn.vfe_init) {
			...
			rc = msm->vfefn.vfe_init(&msm_vfe_s, msm->pdev);
			if (rc < 0) {
				CDBG_ERR("vfe_init failed at %ld\n", rc);
				goto msm_open_proc_done;
			}
			rc = msm->sctrl.s_init(&pdata->sinfo[msm->sidx]);
			if (rc < 0) {
				CDBG_ERR("sensor init at %ld\n", rc);
				goto msm_open_proc_done;
			}
		} else{
				rc = -ENODEV;
...
static int msm_open(struct inode *inode, struct file *filep)
{
	...
	pmsm = container_of(inode->i_cdev, struct msm_device_t, cdev);
...
static long msm_setup_cdevs(struct msm_device_t *msm, dev_t devno)
{
	int rc = -ENODEV;
	...
	CDBG_INFO("msm_camera setup finishes!\n");
	return 0;
...
long msm_v4l2_register(struct msm_driver *drv, const char *id)
{
	long rc = -ENODEV;
	...
	if (drv->vmsm->apps_id == NULL) {
		...
		drv->get_pict  = msm_get_pic_proc;
		drv->drv_poll  = msm_apps_poll;
		rc = 0;

	} else{
		CDBG_ERR("msm register failed msm_camera->apps_id:%s\n",
			drv->vmsm->apps_id);
		rc = -EFAULT;
	}
...
long msm_v4l2_unregister(struct msm_driver *drv, const char *id)
{
	long rc = -EFAULT;
...
int msm_camera_drv_start(struct platform_device *dev)
{
	...
	if (!msm_class) {
		rc = alloc_chrdev_region(&msm_devno, 0, pdata->snum, "msm_camera");
		if (rc < 0) {
			CDBG_ERR("msm_camera: failed to allocate chrdev: %d\n", rc);
			return rc;
		}
		msm_class = class_create(THIS_MODULE, "msm_camera");
		if (IS_ERR(msm_class)) {
			rc = PTR_ERR(msm_class);
			CDBG_ERR("msm_camera: create device class failed: %d\n", rc);
			return rc;
		}
	}

	msm_camera =
		kzalloc(sizeof(struct msm_device_t *) * pdata->snum, GFP_ATOMIC);
	...
	for (i = 0; i < pdata->snum; i++) {
		if (sinfo->sensor_probe(sinfo, &sctrl) >= 0) {
			...
			rc = msm_setup_cdevs(pmsm, MKDEV(MAJOR(msm_devno), cnt));
